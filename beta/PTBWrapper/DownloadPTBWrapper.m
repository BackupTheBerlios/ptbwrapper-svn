...
if downloadmethod <= 1
    checkoutcommand=[svnpath 'svn checkout ' targetRevision ' svn://svn.berlios.de/ptbwrapper/' flavor '/PTBWrapper/ ' pt];
else
    if downloadmethod == 2
        % Good to get through many firewalls and proxies:
        checkoutcommand=[svnpath 'svn checkout ' targetRevision ' https://svn.berlios.de/svnroot/repos/ptbwrapper/' flavor '/PTBWrapper/ ' pt];
    else
        % If firewalls and proxies block everything, one can try this *and*
        % reconfigure its proxy to allow it:
        checkoutcommand=[svnpath 'svn checkout ' targetRevision ' http://svn.berlios.de/svnroot/repos/ptbwrapper/' flavor '/PTBWrapper/ ' pt];
    end
end
